...
function AppointmentList() {
  ...
  const createRdvStaff = async () => {
    const token = localStorage.getItem("token");
    const cleanedToken = token.substring(1, token.length - 1);

    let result = await fetch("http://localhost:8000/rendezvous/createbystaf", {
      method: "post",
      body: JSON.stringify({ date, startTime, endTime, type, price, patient }),
      headers: {
        "Content-Type": "application/json",
        Authorization: cleanedToken,
      },
